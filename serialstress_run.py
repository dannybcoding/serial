...
current_log_file = 'serial_test.log'
backup_log_file = 'serial_test_old.log'

# Function to rotate the log file manually and rename it
def rotate_log():
    if os.path.exists(current_log_file):
        if os.path.exists(backup_log_file):
            os.remove(backup_log_file)  # Remove the old backup file
        os.rename(current_log_file, backup_log_file)  # Rename current log to backup

# Rotate log before starting new session
rotate_log()

# Configure logging with rotation to avoid excessive log size
handler = RotatingFileHandler(current_log_file, maxBytes=5*1024*1024, backupCount=1)  # 5 MB per log file, 1 backups
