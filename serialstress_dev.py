...
# Configure logging to include INFO level messages
logging.basicConfig(filename='serial_test.log', filemode='w', level=logging.DEBUG,
...
def compare_data(sent_data, received_data, port_mapping):
    ...
    if not sent_data:
        logging.warning("Sent data is empty")
...
def stress_test(dut_ports, aux_ports, baudrate, duration_sec, detailed_logging):
    ...
    sent_data = {port: [] for port in dut_ports}
    received_data = {port: [] for port in aux_ports}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--run_until_failure', action='store_true', help='Run the test until a failure is detected')
    ...
    while True:
        ...
        if args.run_until_failure:
            continue  # Continue running until a failure is detected
